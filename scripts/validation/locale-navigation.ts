...
assert.match(htmlFrameSource, /frame\.images === inlineImages && frame\.allowRemote === allowRemote/u)
assert.match(htmlFrameSource, /srcDoc=\{frame\.source\}/u)
assert.doesNotMatch(htmlFrameSource, /srcDoc=\{source\}/u)
assert.match(messageViewSource, /relative min-h-72 flex-1 shrink-0 overflow-auto/u)
assert.match(sharedMessageSource, /relative min-h-72 flex-1 shrink-0 overflow-auto/u)
assert.equal([...threeColumnSource.matchAll(/min-h-0 flex-1 overflow-auto/gu)].length, 3)
assert.equal([...threeColumnSource.matchAll(/<MessageView\b/gu)].length, 1, "only one preview may load remote resources")
assert.equal([...readFileSync(join(process.cwd(), "app/[locale]/shared/[token]/page-client.tsx"), "utf8").matchAll(/<SharedMessageDetail\b/gu)].length, 1)
