...
- `v0.16.6` 的 [Publish Docker Images](https://github.com/DesuwaDev/moemail-local/actions/runs/31595636029) 在原生 amd64/arm64 runner 上全部通过。Web smoke 实际启动 standalone server、health、首次向导、静态文件、WOFF2 字体和 Sharp 图片优化；维护 smoke 实际加载 Linux `better-sqlite3` native binding 与 rclone。amd64 Web 压缩层由 `v0.16.5` 的 290.54 MiB 降至 106.65 MiB（-63.3%），arm64 由 285.94 MiB 降至 107.17 MiB（-62.5%）；未压缩分别为 316.90/335.72 MiB，冷启动为 2/1 秒。按需维护镜像压缩后为 95.79/94.19 MiB，未压缩为 275.92/293.07 MiB。四种 `latest`/`latest-tools` 均与本次 version tag 的 OCI index digest 一致并包含 linux/amd64、linux/arm64。
