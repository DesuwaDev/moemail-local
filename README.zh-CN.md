...
- **请求超时**（默认 `10 秒`）：同时作用于浏览器取题、兑换以及服务端 siteverify 调用，自建服务较慢或较远时可以调大。
- **触感反馈**（默认关闭）：手机点按组件时轻微震动。
- **排障帮助链接**（默认留空）：浏览器拦截组件时，其提示中指向的地址。留空即使用 Cap 官方页面（中国大陆可能打不开）。只接受不含账号密码的 `http(s)` 地址；填错会在保存时被拒绝，且永远不会导致该渠道不可用。
