...
- `ghcr.io/desuwadev/moemail-local:latest-tools` (only for maintenance profiles)
...
- `ghcr.io/desuwadev/moemail-local:latest`
- `ghcr.io/desuwadev/moemail-local:latest-tools` (application maintenance profiles)
- `ghcr.io/desuwadev/moemail-local-postgres:latest`
- `ghcr.io/desuwadev/moemail-local-postgres-tools:latest` (PostgreSQL backup/restore profiles)
...
The Worker must use the exact `email.ingestSecret` generated by the setup wizard. Start with direct forwarding. You can deploy it from a computer with Git, Node.js 24, and Corepack; it does not need to run on the MoeMail server. A Compose-only deployment directory does not contain the Worker source, so these commands fetch the complete matching release:

```bash
git clone --branch v0.21.11 --depth 1 https://github.com/DesuwaDev/moemail-local.git
cd moemail-local
corepack enable
...
**Cap failure handling** (collapsed by default): configure access refusals and network errors with unknown causes separately. Each can stop verification, use the general fallback, or use a specific provider with its own keys. Both default to stopping, including existing configurations without these fields. HTTP 401/403/429/451 and explicit block responses use the refusal policy; CORS errors, disconnections and timeouts use the unknown-cause policy because browsers may hide a regional block's status. Script failures and explicit server outages still use the general fallback. Failed channels are never retried in a loop, and an unavailable explicit target stops verification.

These settings control browser switching, not site-wide geographic access. The backend accepts valid tokens from configured backup providers; enforce strict country restrictions at the application or gateway too. Cap refusal responses should include CORS headers allowing this site's origin to read their status.
...
MoeMail Local derives from [beilunyang/moemail](https://github.com/beilunyang/moemail). Local-first changes are maintained in [DesuwaDev/moemail-local](https://github.com/DesuwaDev/moemail-local); upstream changes should be reviewed and merged deliberately because the runtime and deployment models differ.
